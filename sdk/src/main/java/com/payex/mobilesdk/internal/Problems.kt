...
internal fun Parcel.writeProblem(problem: Problem) = when (problem) {
    ...
    is Problem.Client.PayEx -> writeProperProblem(problem)
    ...
    is Problem.Server.PayEx -> writeProperProblem(problem)
...
private fun parsePayExProblem(
    constructor: (JsonObject, String?, Int, String?, String?, PayexAction?, List<PayExSubproblem>) -> Problem,
    status: Int,
    json: JsonObject
): Problem {
    ...
    object Client : ProblemSpace() {
        override fun parseKnownProblem(type: String, status: Int, json: JsonObject) = when (type) {
            ...
            "https://api.payex.com/psp/errordetail/inputerror" ->
                parsePayExProblem(Problem.Client.PayEx::InputError, status, json)
            "https://api.payex.com/psp/errordetail/forbidden" ->
                parsePayExProblem(Problem.Client.PayEx::Forbidden, status, json)
            "https://api.payex.com/psp/errordetail/notfound" ->
                parsePayExProblem(Problem.Client.PayEx::NotFound, status, json)
            else -> null
        }
    ...
    object Server : ProblemSpace() {
        override fun parseKnownProblem(type: String, status: Int, json: JsonObject) = when (type) {
            ...
            "https://api.payex.com/psp/errordetail/configurationerror" ->
                parsePayExProblem(Problem.Server.PayEx::ConfigurationError, status, json)
            else -> null
        }
